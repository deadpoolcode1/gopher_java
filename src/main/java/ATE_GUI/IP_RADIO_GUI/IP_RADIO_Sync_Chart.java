...
public class IP_RADIO_Sync_Chart extends XY_Graph_Template {
    ...
    public IP_RADIO_Sync_Chart(JPanel owner, int width, int height, String chartTitle,
        ...
        iP_RADIO_GD.SYNC_SIGNAL_PWR = new XYSeries("SYNC_SIGNAL_PWR");  dataset.addSeries(iP_RADIO_GD.SYNC_SIGNAL_PWR);
        iP_RADIO_GD.SYNC_NOISE_PWR = new XYSeries("SYNC_NOISE_PWR");  dataset.addSeries(iP_RADIO_GD.SYNC_NOISE_PWR);
    }
    ...
    public void UpdateDataset(int timein, double sync_signal_pwr, double sync_noise_pwr) {
        ...
        iP_RADIO_GD.SYNC_SIGNAL_PWR.add(time, sync_signal_pwr);
        if(iP_RADIO_GD.SYNC_SIGNAL_PWR.getItemCount() > 100) iP_RADIO_GD.SYNC_SIGNAL_PWR.remove(0);
        iP_RADIO_GD.SYNC_NOISE_PWR.add(time, sync_noise_pwr);
        if(iP_RADIO_GD.SYNC_NOISE_PWR.getItemCount() > 100) iP_RADIO_GD.SYNC_NOISE_PWR.remove(0);
    }
}
